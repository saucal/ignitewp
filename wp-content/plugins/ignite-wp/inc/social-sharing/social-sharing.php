	    	...
	    	if ( false === ( $post_share_count = get_transient( 'post_'.$post_id.'_share_count' ) ) ) {
				...
				if( !SocialCount::REQUIRE_LOCAL_URL || SocialCount::isLocalUrl( $_GET['url'] ) ) {
					try {
						...
						$post_share_count = array("error" => $e->getMessage());
					}
				} else { 
					$post_share_count = array("error" => 'URL not authorized (' . $_SERVER['HTTP_HOST'] . ')');
				}
...
function get_post_socials_ajax() {
    ...
    } else if(isset($_REQUEST["post_url"])) {
...
add_filter("posts_clauses", function($clauses, $wpq) {
	//$clauses["orderby"]
